    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-[#062B24]">{t('إصدارات المتجر', 'Store Books')}</h2>
            <p className="text-sm text-[#8B9D8A]">{t('إدارة الكتب المطبوعة والرقمية المعروضة للبيع', 'Manage printed and digital books for sale')}</p>
          </div>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 bg-[#062B24] text-white rounded-xl text-sm font-bold hover:bg-[#1B4D42] transition-colors">
          <Plus size={16} /> {t('إضافة كتاب', 'Add Book')}
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left" dir={isRTL ? 'rtl' : 'ltr'}>
            <thead className="bg-[#F8F4EA] text-[#5A7A70] uppercase text-xs">
              <tr>
                <th className="px-6 py-4">{t('الكتاب', 'Book')}</th>
                <th className="px-6 py-4">{t('السعر', 'Price')}</th>
                <th className="px-6 py-4">{t('المخزون', 'Stock')}</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {books.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-[#8B9D8A]">{t('لا توجد كتب', 'No books found')}</td></tr>
              ) : books.map(b => (
                <tr key={b._id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-bold text-[#062B24]">{isRTL ? b.title_ar : b.title_en}</td>
                  <td className="px-6 py-4">AED {b.salePrice || b.price}</td>
                  <td className="px-6 py-4">{b.stock}</td>
                  <td className="px-6 py-4">{b.isFeatured ? <Check size={16} className="text-green-500"/> : <X size={16} className="text-red-500"/>}</td>
